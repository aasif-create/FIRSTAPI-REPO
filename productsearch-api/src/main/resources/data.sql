INSERT INTO items (name, category, price, rating, image_url, available) VALUES
('Idly', 'South Indian', 30.0, 4.5, 'https://via.placeholder.com/120?text=Idli', true),
('Masala Dosa', 'South Indian', 70.0, 4.7, 'https://via.placeholder.com/120?text=Dosa', true),
('Sambar Vada', 'South Indian', 45.0, 4.2, 'https://via.placeholder.com/120?text=Vada', true),
('Chicken Biryani', 'Biryani', 150.0, 4.8, 'https://via.placeholder.com/120?text=Biryani', true),
('Paneer Butter Masala', 'North Indian', 140.0, 4.6, 'https://via.placeholder.com/120?text=Paneer', true),
('Poori', 'North Indian', 40.0, 4.1, 'https://via.placeholder.com/120?text=Poori', true),
('Rasam Rice', 'South Indian', 60.0, 4.0, 'https://via.placeholder.com/120?text=Rasam', true),
('Hyderabadi Biryani', 'Biryani', 180.0, 4.9, 'https://via.placeholder.com/120?text=Hyderabadi', true),
('Uttapam', 'South Indian', 80.0, 4.3, 'https://via.placeholder.com/120?text=Uttapam', true),
('Gulab Jamun', 'Dessert', 50.0, 4.4, 'https://via.placeholder.com/120?text=Gulab+Jamun', true);

INSERT INTO items (name, category, price, rating, image_url, available)
VALUES ('Idli', 'South Indian', 30.0, 4.5, '/images/idly.jpg', true);



